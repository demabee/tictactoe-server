...
const chain_execute_1 = require("./util/chain-execute");
class SocketControllers {
    ...
    registerController(socket, controller) {
        ...
        for (const messageAction of messageActions) {
            socket.on(messageAction.options.name, (...args) => {
                ...
                void this.executeAction(socket, controller, messageAction, messageAction.options.name, messages, ack);
            });
        }
    }
    async executeAction(socket, controller, action, eventName, data, ack) {
        const eventContext = this.resolveEventContext(socket, action.type, eventName, data, controller.metadata.namespace, ack);
        ...
        try {
            ...
            const actions = [
                ...(action.interceptors || []).map(interceptor => {
                    return (interceptor instanceof Function
                        ? container.get(interceptor)
                        : interceptor).use.bind(interceptor);
                }),
                (context) => {
                    const parameters = this.resolveParameters(socket, controller.metadata, action.parameters || [], context.messageArgs, ack);
                    return controllerInstance[action.methodName](...parameters);
                },
            ];
            const actionResult = (0, chain_execute_1.chainExecute)(eventContext, actions);
    ...
    resolveParameters(socket, controllerMetadata, parameterMetadatas, data, ack) {
        const parameters = [];
        for (const metadata of parameterMetadatas) {
            const parameterValue = this.resolveParameter(socket, controllerMetadata, metadata, data, ack);
            parameters[metadata.index] = this.transformActionValue(parameterValue, metadata.reflectedType, metadata.options, 'parameter');
        }
        return parameters;
    }
    resolveParameter(socket, controller, parameter, data, ack) {
        var _a;
        switch (parameter.type) {
            ...
            case ParameterType_1.ParameterType.MESSAGE_ACK:
                return ack;
    ...
    resolveEventContext(socket, eventType, eventName, messageBody, namespace, ack) {
        return {
            eventType,
            eventName,
            socket,
